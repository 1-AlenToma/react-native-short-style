...
export const FormGroup = (props: FormGroupProps & { children: (React.ReactElement<FormItemProps> | React.ReactElement<FormItemProps>[]) }) => {
    ...
    return (
        <ForGroupContext.Provider value={{ labelPosition: props.labelPosition }}>
            <View style={props.style} css={x => x.cls("_formGroup").maT(30).joinRight(props.css)}>
                ...
                <View>
                    <View>
                        <View ifTrue={props.title != undefined && props.formStyle != "Headless"} css="wi-100% fl-0 flg-1 pa-5 header">
                            <Text numberOfLines={1}>{props.title}</Text>
                        </View>
                        <View css={x => x.maT(8).fillView().flG(1).paL(5).paR(5)}>
                            {
                                props.children
                            }
                        </View>
