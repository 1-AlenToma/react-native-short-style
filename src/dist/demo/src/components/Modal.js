...
export const Modal = (props) => {
    ...
    const render = () => {
        ...
        if (state.isVisible) {
            context.add(state.id, React.createElement(View, { key: state.id, css: "_blur op:1 bac:transparent fl:1", style: { zIndex: zIndex + 300 } },
                React.createElement(Blur, { style: {
                        opacity: animate.y
                    }, onPress: props.disableBlurClick ? undefined : () => {
                        toggle(false);
                    }, css: "_blur zi:1" }),
                React.createElement(AnimatedView, Object.assign({}, props, { css: x => x.cls("_modalDefaultStyle zi:2 _modal sh-sm _overflow Modal").joinRight(props.css), style: [...style,
                        {
                            transform: transform.scale ? [transform] : undefined,
                            opacity: transform.opacity ? transform.opacity : undefined
                        }
                    ] }),
                    React.createElement(View, { ifTrue: props.addCloser == true, css: x => x.cls("_modalClose").baC("$co-transparent") },
                        React.createElement(Button, { onPress: () => toggle(false), css: x => x.cls("sh-none", "_center").size(25, 25).baC("$co-transparent").juC("flex-end").pa(0).paL(1).boW(0), icon: React.createElement(Icon, { type: "AntDesign", name: "close", size: 15 }) })),
                    React.createElement(View, { css: x => x.fillView().zI(1).baC("$co-transparent").if(props.addCloser == true, x => x.maT(Platform.OS == "web" ? 5 : 10)) }, props.children))));
        }
        else {
