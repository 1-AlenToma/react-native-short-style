...
export const ButtonGroup = (props) => {
    ...
    if (numColumns === 0)
        numColumns = 1;
    return (_jsx(View, { ifTrue: props.ifTrue, css: x => x.cls("_buttonGroup", "ButtonGroup").joinRight(props.css), style: props.style, children: props.scrollable ? (_jsx(VirtualScroller, { ...cProps, numColumns: numColumns, itemSize: props.itemSize, items: props.buttons, renderItem: ({ item, index }) => getItem(item, index), horizontal: !props.isVertical, showsHorizontalScrollIndicator: false, showsVerticalScrollIndicator: false, ref: c => state.scrollView = c, updateOn: [...state.selectedIndex, ...(props.updateOn ?? [])] })) : (_jsx(View, { ...cProps, children: _jsx(View, { style: props.style, css: x => x.cls("_buttonGroupCenter").if(props.isVertical, c => c.flD("column"), c => c.flD("row")).joinRight(props.css), children: props.buttons.map((x, index) => getItem(x, index)) }) })) }));
};
