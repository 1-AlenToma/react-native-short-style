...
export const ActionSheet = (props) => {
    ...
    const renderUpdate = () => {
        ...
        fn(state.id, _jsxs(View, { css: "co-transparent _topPostion", style: { zIndex: zIndex + 300 }, children: [_jsx(Blur, { style: {
                        ...
                        if (!props.disableBlurClick)
                            toggle(false);
                    }, css: "zi:1" }), _jsx(AnimatedView, Object.assign({ onTouchStart: () => {
                        //state.refItem.isTouched = true;
                    }, onTouchEnd: (event) => {
                    ...
                    ] }, state.refItem.panResponse.panHandlers, { children: _jsxs(View, { style: {
                            flexDirection: !isVertical ? "row" : undefined
                        }, css: "wi:100% he:100% pa:10 flex:1", children: [position == "Bottom" || position == "Right" ? handle : null, _jsx(View, { ifTrue: state.refItem.show || !props.lazyLoading, css: x => x.joinLeft("zi:5 maw:99% _overflow mat:5 bac-transparent").joinRight(props.css).zI(5).importantValue(), style: [(props.size != "content" ? {
                                        flex: 1,
                                        flexGrow: 1
