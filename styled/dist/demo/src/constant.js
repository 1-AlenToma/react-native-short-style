 ...
 */
    static async runInChunksAndGroupAsync(items, chunkSize, callback) {
        let index = 0;
        const processChunk = async () => {
            ...
            if (index < items.length) {
                await new Promise((r) => SmartScheduler.run(r));
                await processChunk();
            }
        };
        await processChunk();
     ...
     * Same as runInChunks, but awaits each chunk (useful if chunk processing is async)
     */
    static async runInChunksAsync(items, chunkSize, callback) {
        let index = 0;
        const processChunk = async () => {
            const end = Math.min(index + chunkSize, items.length);
            for (; index < end; index++) {
